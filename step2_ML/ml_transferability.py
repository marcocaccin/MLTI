...
idx_train, idx_test = get_train_test_idx(X1, n_interp, do_clustering=False, do_birch=False)
...
idx_train, idx_test = get_train_test_idx(X2, n_interp, do_clustering=False, do_birch=False)
...
y_ML2 = model1.predict(xgb.DMatrix(X2))
y_ML1 = model2.predict(xgb.DMatrix(X1))
...
plt.tight_layout()
plt.legend()
plt.savefig("f_ml_transfer_%d.pdf" % n_interp)
