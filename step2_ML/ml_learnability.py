...
X, y, y_TS, colvar = load_data('amorph')
n_data = len(y)
n_interp = 50
...
iy, _ = plot_the_integral(y, colvar, label='DFT', color=colors[0])
y_DFT_interp = np.interp(colvar, colvar[::n_interp], y[::n_interp])
iy_DFTinterp, _ = plot_the_integral(y_DFT_interp, colvar, label='DFT interpolation', color=colors[1])
# iy_TS, _ = plot_the_integral(y_TS, colvar, label='TS', color=colors[1])
iy_ML, _ = plot_the_integral(y_ML, colvar, label='ML', color=colors[2])

plt.legend()
plt.xlim(colvar.min(), colvar.max())
# plt.ylim(-1, 1.7)
plt.ylim(-3, 1)
